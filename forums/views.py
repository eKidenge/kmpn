...
from accounts.decorators import user_activity_log, role_required
...
def forum_list(request):
    ...
    if search_form.is_valid():
        ...
        # Only apply ordering if sort_by is not empty
        if sort_by:
...
def thread_detail(request, thread_id):
    ...
    # Check if user is authenticated for forms
    reply_form = ForumReplyForm() if request.user.is_authenticated and can_reply else None
...
def edit_thread(request, thread_id):
    ...
    if request.method == 'POST':
        ...
        initial = {}
        if thread.tags:
            initial['tags'] = ', '.join(thread.tags)
        form = ForumThreadForm(instance=thread, initial=initial)
...
def delete_thread(request, thread_id):
    ...
    context = {
        'thread': thread,
        'page_title': f'Delete {thread.title}',
    }
    return render(request, 'forums/delete_thread.html', context)
...
def report_content(request, content_type, content_id):
    ...
    redirect_url = 'forums:thread_detail'
    redirect_id = content.id if content_type == 'thread' else content.thread.id
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(redirect_url, thread_id=redirect_id)
    else:
...
def manage_threads(request):
    """Manage forum threads (admin only)"""
    ...
    if request.method == 'POST':
        ...
        if action == 'pin':
            ...
            thread.save()
            messages.success(request, 'Thread archived successfully.')
        elif action == 'restore':
            thread.status = 'open'
            thread.save()
            messages.success(request, 'Thread restored successfully.')
        
        return redirect('forums:manage_threads')
    
    context = {
        'threads': threads,
        'status_filter': status_filter,
        'search_query': search_query,
        'page_title': 'Manage Threads - Admin',
    }
    return render(request, 'forums/manage_threads.html', context)
        ...
        'replies': [
            {
                ...
                'like_count': reply.like_count,
                'is_author': request.user == reply.author if request.user.is_authenticated else False,
            }
            for reply in replies
        ],
        'has_next': replies.has_next(),
        'has_previous': replies.has_previous(),
        'current_page': replies.number,
        'total_pages': replies.paginator.num_pages,
    }
...
@login_required
def search_threads_ajax(request):
    """Search threads via AJAX"""
    ...
    for thread in threads:
        results.append({
            ...
            'author': thread.author.get_full_name() or thread.author.username,
            'reply_count': thread.reply_count,
            'created_at': thread.created_at.strftime('%Y-%m-%d'),
            'url': f'/forums/thread/{thread.id}/'
        })
