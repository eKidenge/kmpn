# communities/views.py - COMPLETE FIXED VERSION
...
def community_detail(request, slug):
    ...
    # Get posts - FIXED: Use the model's Meta ordering or order by status
    ...
    # Sort posts - FIXED: Use Case/When for pinned status ordering
    ...
    if sort_by == 'popular':
        ...
        posts = posts.annotate(
            is_pinned=Case(
                When(status='pinned', then=Value(1)),
                default=Value(0),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if post.status == 'published':
                _notify_members_about_post(community, post)
...
def _notify_members_about_post(community, post):
